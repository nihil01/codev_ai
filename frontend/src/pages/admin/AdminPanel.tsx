      ...
      <div className="space-y-6">
        <div className="grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
          <motion.section initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className={cardClass}>
            <h2 className="text-xl font-semibold text-[#020520]">Administrator</h2>
          ...
          <motion.section initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.06 }} className={cardClass}>
            <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
              <div>
                <h2 className="text-xl font-semibold text-[#020520]">{t('admin.createCompany')}</h2>
                <p className="mt-1 text-sm text-[#696a72]">{t('admin.createCompanyHint')}</p>
              </div>
            </div>
            ...
            <form onSubmit={handleCreateCompanyUser} className="grid gap-4">
              ...
              <Field label={t('companyInfo.businessType')}>
                ...
                <p className="mt-2 text-xs font-semibold text-[#696a72]">
            ...
            <AnimatePresence>
              {createdUser && (
                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="mt-6 rounded-lg border border-[#d1fadf] bg-[#f0fdf4] p-5 text-sm text-[#166534]">
                  <h3 className="text-lg font-semibold text-[#020520]">{t('admin.companyCreated')}</h3>
                  ...
                  <p className="mt-4 font-semibold text-[#166534]">{t('admin.saveAndSend')}</p>
                </motion.div>
              )}
            </AnimatePresence>
          </motion.section>
        ...
        <motion.section initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className={cardClass}>
          <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.24em] text-[#145aff]">AI Prompts</p>
              <h2 className="mt-2 text-xl font-semibold text-[#020520]">Client AI Settings</h2>
              <p className="mt-1 text-sm text-[#696a72]">{t('admin.promptHint')}</p>
            </div>
          </div>
          ...
          <div className="grid gap-5 lg:grid-cols-[0.8fr_1.2fr]">
            <div className="space-y-4">
              ...
              {promptRecord && (
                <div className="rounded-lg border border-[#e2e4e9] bg-[#fcfcfc] p-4 text-sm">
              ...
              {subscription && (
                <form onSubmit={saveSubscription} className="rounded-lg border border-[#e2e4e9] bg-[#fcfcfc] p-4 text-sm">
                  <h3 className="text-base font-semibold text-[#020520]">Package and access</h3>
                  <div className="mt-4 grid gap-3">
                    ...
                    <label className="flex items-center gap-3 rounded-lg border border-[#e2e4e9] bg-white px-4 py-3 font-semibold text-[#020520]">
            ...
            <form onSubmit={savePrompt} className="space-y-4">
              {loadingPrompt ? (
                <div className="rounded-lg border border-[#e2e4e9] bg-[#fcfcfc] p-5 text-[#696a72]">
                ...
                <>
                  ...
                  <div className="rounded-lg border border-[#e2e4e9] bg-[#f0f4fe] p-4">
                    <div className="mb-4 flex items-center justify-between gap-3">
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#145aff]">Instagram Comments</p>
                        <h3 className="text-base font-semibold text-[#020520]">Comment system prompt</h3>
                      </div>
                      {commentPromptRecord && <span className="rounded-lg bg-white border border-[#e2e4e9] px-3 py-1 text-xs font-semibold text-[#696a72]">v{commentPromptRecord.version}</span>}
                    </div>
                    <div className="space-y-4">
