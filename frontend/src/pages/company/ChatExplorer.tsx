...
function channelBadgeClass(channel: Conversation['channel']) {
  return channel === 'whatsapp' ? 'bg-[#e4f5e9] text-[#18261d]' : 'bg-[#e4f5e9] text-[#116932]';
}
...
function modeBadgeClass(mode: Conversation['mode']) {
  if (mode === 'human') return 'bg-[#ffffff] text-[#18261d]';
  if (mode === 'paused') return 'bg-[#ffffff] text-[#18261d]';
  if (mode === 'closed') return 'bg-[#d8e8dd] text-[#18261d]';
  return 'bg-[#e4f5e9] text-[#116932]';
}
    ...
    <section className="space-y-6">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className={`${cardClass} space-y-5`}>
        <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.25em] text-[#18261d]">{t('chat.eyebrow')}</p>
            <h2 className="mt-2 text-2xl font-light text-[#18261d]">{t('chat.title')}</h2>
          </div>
          <div className="rounded-[24px] border border-[#e1ebe4] bg-[#ffffff] px-4 py-3 text-sm font-semibold text-[#18261d]">
                ...
                return (
                  <button
                    ...
                    className={`w-full rounded-[24px] border p-4 text-left transition duration-200 ${
                      active
                        ? 'border-[#18261d] bg-[#e4f5e9]'
                        : 'border-[#e1ebe4] bg-[#ffffff] hover:border-[#15803d]/40 hover:bg-[#e4f5e9]/50'
                    }`}
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          <p className="truncate font-semibold text-[#18261d]">{customerLabel(conversation)}</p>
                        ...
                        <p className="mt-1 truncate text-xs text-[#18261d]">{conversation.external_conversation_id}</p>
                      </div>
                      <span className="rounded-full bg-[#e4f5e9] px-3 py-1 text-xs font-semibold text-[#18261d]">
                        {conversation.messages.length}
                      </span>
                    </div>
                    <p className="mt-3 line-clamp-2 text-sm leading-6 text-[#18261d]">
                      {lastMessage?.text || t('chat.noMessages')}
                    </p>
                    <p className="mt-3 text-xs font-medium text-[#18261d]">{formatDate(conversation.last_message_at || conversation.created_at)} · {t('chat.window')}: {windowLeftLabel(conversation.messaging_window_expires_at)}</p>
                  </button>
                );
              })}
            </div>
        ...
        <motion.div initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} className={`${cardClass} min-h-[520px]`}>
          {!selectedConversation ? (
            ...
            <div className="flex h-full flex-col">
              <div className="border-b border-[#e1ebe4] pb-4">
                <div className="flex flex-wrap items-center gap-2">
                  <h3 className="text-xl font-light text-[#18261d]">{customerLabel(selectedConversation)}</h3>
                      ...
                      >
                        <div
                          className={`max-w-[78%] rounded-[24px] px-4 py-3 text-sm leading-6 ${
                            outbound
                              ? 'bg-[#18261d] text-[#ffffff]'
                              : 'border border-[#e1ebe4] bg-[#ffffff] text-[#18261d]'
                          }`}
                        >
                          <p className="whitespace-pre-wrap">{message.text || '—'}</p>
                          <p className={`mt-2 text-[11px] font-medium ${outbound ? 'text-[#18261d]' : 'text-[#18261d]'}`}>
              ...
              <form
                className="mt-5 rounded-[24px] border border-[#e1ebe4] bg-[#ffffff] p-4"
                onSubmit={(event) => {
                  event.preventDefault();
                  void sendManualMessage();
                }}
              >
                <label className="text-xs font-semibold uppercase tracking-[0.16em] text-[#18261d]" htmlFor="manual-chat-message">
                  {t('chat.replyLabel')}
                </label>
                <textarea
                ...
                <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <p className="text-xs font-medium text-[#18261d]">
